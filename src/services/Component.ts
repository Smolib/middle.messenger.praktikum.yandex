...
export default class Component {
  ...
  createDocumentElement(tag) {
    ...
    if (this._props.settings?.whitInternalID) {
  ...
  compile(template, props) {
    ...
    propsAndStubs["dynamicChildren"] = dynamicChildren;

    const fragment = this.createDocumentElement('template');

    fragment.innerHTML = Handlebars.compile(template)(propsAndStubs);

    const replaceStub = (child) => {
    ...
    Object.values(this._children).forEach(replaceStub);
    Object.values(dynamicChildren).forEach(replaceStub);
